            ...
            if ($db->rowCount() > 0) {
                ...
                if ($db->rowCount() > 0) {
                    ...
                    foreach ($get_student as $row) {
                        ...
                        if ($db->rowCount() > 0) {
                            ...
                            foreach ($results as $result) {
                                ...
                                if ($db->rowCount() > 0) {
                                    ...
                                    // {
                                    //DIVIDING BY 3
                                    $pdf->Cell(13, 5, round(($row->sess_avg_sum), 2, PHP_ROUND_HALF_UP), 1, 0, 'C');
                                    // }    
                                } else {
                        ...
                        $db->execute();
                        if ($db->rowCount() > 0) {
                            $second_term_total = $single()->second_total_sum;
    ...
    /* if($term_name == "THIRD TERM")
    {
        ...
        else
        {
            if(mysqli_num_rows($sql_run) > 0)
            {
                ...
                $get_student = mysqli_query($con, "SELECT * FROM students_tbl WHERE class_name='$class_name'");
                if(!$get_student)
                {
                    die("Query failed ".mysqli_error($con));
                }
            ...
            if (mysqli_num_rows($sql_run) > 0) {
                ...
                if (!$get_student) {
                    ...
                    if ($get_student) {
                        ...
                        //Creating downloadable file as pdf
                        $download = $class_name . ' (' . $session_name . ' - ' . $term_name . ')';
